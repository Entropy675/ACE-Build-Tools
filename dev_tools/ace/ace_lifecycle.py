...
class LifecycleMixin:
    ...
    def create_script(self, script_name):
        """Create a new .etcs script file with shebang, make executable, and open in editor."""
        # Sanitize the name but allow hyphens for script filenames (more permissive than modules)
        clean_name = re.sub(r'[^a-zA-Z0-9_\-]', '', script_name.strip())
        if not clean_name:
            print(f"[-] Error: Invalid script name '{script_name}'")
            return

        filepath = Path.cwd() / f"{clean_name}.etcs"

        if filepath.exists():
            print(f"[-] Error: '{filepath.name}' already exists at {filepath}")
            return

        # UPDATED: Now uses /usr/bin/env to dynamically find the etcs runtime in the PATH
        shebang = "#!/usr/bin/env etcs\n"
        filepath.write_text(shebang)
