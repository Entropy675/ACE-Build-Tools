...
class ManifestMixin:
    ...
    def _emit_loaders_makefile(self, default, overrides, silent=False):
        ...
        w("endif")
        w("")
        w("# WHERE WEB ARTIFACTS LAND, and the reason is that a served directory")
        w("# should contain nothing but what is meant to be served.")
        w("#")
        w("# bin/ holds the native runtime, the .so modules, run_all_tests.sh and")
        w("# the shader assets. Serving bin/ to reach etcs.wasm exposes all of it;")
        w("# the alternative was what every page did instead -- copy the web")
        w("# artifacts into its own www/ after each build, which is a step to")
        w("# forget and a stale binary when you do. bin/wasm/ is one directory a")
        w("# script can mount whole, and `ace wasm make ...` is the only thing")
        w("# that writes to it.")
        w("#")
