...
class ManifestMixin:
    ...
    def _emit_loaders_makefile(self, default, overrides, silent=False):
        ...
        w("    #")
        w("    # 512 MB, from 256: the runtime's arenas alone stood at ~230 MB after a")
        w("    # paint page booted (measured with malloc_footprint from the console),")
        w("    # which left a two-layer 1216x960 canvas -- 9 MB of raster -- to be the")
        w("    # allocation that aborted. Pages are committed by the browser as they")
        w("    # are touched, so the larger number costs an untouched tab nothing.")
        w("    ETCS_WEB_MEMORY ?= -sINITIAL_MEMORY=536870912")
        w("")
